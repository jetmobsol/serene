...
Full documentation from the [playwright-cli](https://github.com/microsoft/playwright-cli) project.
...
```
playwright-cli press <key>             # press a key on the keyboard
...
The CLI can be configured using `playwright-cli.json` in the working directory, or via `--config path/to/config.json`.
