...
8. For each story, build its `SCREENSHOT_PATH` by combining three parts:
   - `RUN_DIR` (from step 7)
   - Source file stem (filename without `.yaml` extension, e.g. `auth.yaml` → `auth`)
   - Slugified story name (lowercase, replace spaces with hyphens, e.g. `"Login page loads"` → `login-page-loads`)

   Concatenate: `SCREENSHOT_PATH = "{RUN_DIR}/{file-stem}/{slugified-name}/"`

   Example: `screenshots/bowser-qa/20260210_143022_a1b2c3/auth/login-page-loads/`
