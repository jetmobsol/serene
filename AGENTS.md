...
```bash
# Task runner (recommended entry point)
just start                     # DB in Docker + native dev servers
just stop                      # Stop dev servers and DB
just docker-start              # Full Docker Compose stack
just docker-stop               # Stop all Docker services

# Bun commands (used by just recipes internally)
