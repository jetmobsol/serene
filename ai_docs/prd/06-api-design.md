...
4. Return `tags` directly (stored as PostgreSQL text array — no parsing needed).
...
z.object({
  id: z.string(),
  mood: z
    .enum(["Happy", "Calm", "Anxious", "Sad", "Overwhelmed", "Angry"])
    .optional(),
  tags: z
    .array(
      z.enum([
...
**Behavior:** Use PostgreSQL `unnest(tags)` to expand tag arrays in SQL, then aggregate with `GROUP BY` to calculate average mood score per tag. Filter to tags with >= 3 entries. Runs entirely in the database — no application-level parsing needed.
